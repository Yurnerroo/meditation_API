...
def init_logger(
    app_name: str | None = None, log_level: str | None = None
) -> logging.Logger:
    log_destinations = [
        destination.lower() for destination in settings.LOG_DESTINATIONS
    ]
...
def _get_file_rotating_handler():
    ...
    handler = TimedRotatingFileHandler(
        "logs/backend.log", when="midnight", backupCount=3
    )
